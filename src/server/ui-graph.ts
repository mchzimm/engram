...
export function buildGraphScript(): string {
  return [
    ...
    "    };",
    "",
    "    const relaxPasses = layoutMode === 'stream' ? (N > 400 ? 1 : 2) : (N > 400 ? 4 : 5);",
    ...
    "    }",
    "",
    "    const finalOverlapPasses = layoutMode === 'stream' ? 4 : 24;",
    "    let finalOverlap = 0;",
    ...
    "    try {",
    "      const quickIters = Math.max(1, Math.min(3, Math.ceil(sim.length / 30)));",
    ...
    "  function tick() {",
    "    if (!running) return;",
    "    // Layout is fully settled during pre-relaxation; the loop is a redraw pump only.",
    "    draw();",
    "    frames++;",
    ...
    "      if (!isCurrent()) return;",
    "      if (addNode(master)) {",
    "        addEdgesFor(master.id);",
    "        const settled = await settleVisible('master ' + (master.label || master.id), scale);",
    ...
    "        addEdgesFor(node.id);",
    "        const settled = await settleVisible('round ' + (roundIndex + 1) + '/' + rounds.length, scale);",
    "        scale = Math.max(scale, settled.scale);",
    "        await sleep(10);",
    "      }",
    ...
    "        if (!addNode(node)) continue;",
    "        addEdgesFor(node.id);",
    "        const settled = await settleVisible('orphan cluster', scale);",
    "        scale = Math.max(scale, settled.scale);",
    "        await sleep(10);",
    "      }",
    "    }",
    "",
    "    if (!isCurrent()) return;",
    "    let backupScale = Math.max(1, Math.min(2.5, scale));",
    "    let backupMetrics = null;",
    "    for (let attempt = 0; attempt < 6; attempt++) {",
    "      window.__engram_graph_layoutMode = 'settle-first';",
    "      window.__engram_graph_layoutScale = backupScale;",
    "      renderGraph(canvas, safeNodes, godEntries, safeEdges);",
    "      backupMetrics = window.__engram_graph_layoutMetrics || null;",
    "      try {",
    "        renderMemoryLegend(safeNodes, safeEdges, safeNodes.length);",
    "      } catch (e) {",
    "        // ignore final legend failures",
    "      }",
    "      updateInfo(backupMetrics, 'settle-first backup');",
    "      if (!backupMetrics || backupMetrics.overlapCount === 0) break;",
    "      backupScale = Math.min(2.5, backupScale + 0.12);",
    "      await sleep(10);",
    "    }",
