...
export function buildGraphScript(): string {
  return [
    ...
    "const NODE_SHELLS = {",
    "  project: { radius: 22, alpha: 1.0, size: 8.0, pull: 0.024 },",
    "  god: { radius: 64, alpha: 0.95, size: 6.6, pull: 0.017 },",
    "  child: { radius: 108, alpha: 0.82, size: 5.0, pull: 0.011 },",
    "  edge: { radius: 150, alpha: 0.68, size: 4.0, pull: 0.007 },",
    "};",
    ...
    "    project: Math.max(24, shellScale * 0.04),",
    "    god: Math.max(72, shellScale * 0.11),",
    "    child: Math.max(122, shellScale * 0.18),",
    "    edge: Math.max(170, shellScale * 0.25),",
    "  };",
    "",
    "  function getScope(n) {",
    "    try {",
    ...
    "        const radialJitter = seededNoise(root + \":\" + n.id + \":\" + tier);",
    "        const localRadius = baseRadius * (radialSpread + radialJitter * 0.28);",
    "        const microJitter = tier === \"project\" ? 1.8 : tier === \"god\" ? 3.2 : tier === \"child\" ? 4.8 : 6.5;",
    "        const x = cx + Math.cos(angle) * localRadius + (seededNoise(root + \"::x::\" + n.id + \":\" + tier) - 0.5) * microJitter;",
    "        const y = cy + Math.sin(angle) * localRadius + (seededNoise(root + \"::y::\" + n.id + \":\" + tier) - 0.5) * microJitter;",
    "        sim.push({",
    "          id: n.id,",
    "          label: n.label || n.id,",
    "          kind: n.kind || \"default\",",
    ...
    "    const base = Math.max(SPRING_LENGTH, nodeRadiusFor(a) + nodeRadiusFor(b) + 12);",
    ...
    "      const minDist = nodeRadiusFor(a) + nodeRadiusFor(b) + 12;",
    "      pairCount++;",
    ...
    "  const DAMPING = 0.88;",
    "  const GLOBAL_GRAVITY = 0.00055;",
    "  const CLUSTER_GRAVITY = 0.00135;",
    "  const COLLISION_GAP = 12;",
    "  const COLLISION_FORCE = 0.18;",
    ...
    "              const minDist = nodeRadiusFor(a) + nodeRadiusFor(b) + COLLISION_GAP;",
    "              if (dist < minDist) force += (minDist - dist) * COLLISION_FORCE;",
    ...
    "    const extraIters = layoutMetrics.overlapCount > 0 || layoutMetrics.meanEdgeStretch > 0.32 || layoutMetrics.meanShellError > 0.24",
    "      ? Math.min(240, Math.max(40, Math.round(layoutMetrics.overlapCount * 5 + layoutMetrics.meanEdgeStretch * 140 + layoutMetrics.meanShellError * 90)))",
    "      : 0;",
