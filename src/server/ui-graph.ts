...
export function buildGraphScript(): string {
  return [
    ...
    "",
    "    const clampNode = (n) => {",
    "      const pad = Math.max(18, Math.min(64, nodeCollisionRadius(n) * 0.4 + 10));",
    ...
    "      finalOverlap = resolveCollisionsPass(2.0 + pass * 0.12);",
    "      for (const n of sim) clampNode(n);",
    ...
    "    try {",
    "      const quickIters = Math.max(2, Math.min(8, Math.ceil(sim.length / 20)));",
    ...
    "    };",
    "",
    "    const renderCurrent = (mode, scale) => {",
    "      if (!isCurrent()) return null;",
    "      window.__engram_graph_layoutMode = mode;",
    "      window.__engram_graph_layoutScale = scale;",
    ...
    "        metrics = renderCurrent('stream', scale);",
    "        updateInfo(metrics, stage);",
    "        if (!isCurrent()) return { metrics: null, scale };",
    "        if (!metrics || metrics.overlapCount === 0) break;",
    "        scale = Math.min(2.5, scale + 0.12 + Math.min(0.08, metrics.overlapCount * 0.01));",
    "        await sleep(10);",
    "      }",
    "      return { metrics, scale };",
    "    };",
    "",
    "    let scale = Math.max(1.05, masters.length > 1 ? 1.12 : 1.0);",
    ...
    "        addEdgesFor(node.id);",
    "        const settled = await settleVisible('round ' + (roundIndex + 1) + '/' + rounds.length, Math.max(scale, 1 + roundIndex * 0.05));",
    "        scale = Math.max(scale, settled.scale);",
    "        await sleep(10);",
    "      }",
    "      const seal = await settleVisible('round ' + (roundIndex + 1) + '/' + rounds.length + ' seal', Math.max(scale, 1 + roundIndex * 0.05));",
    ...
    "        if (!addNode(node)) continue;",
    "        addEdgesFor(node.id);",
    "        const settled = await settleVisible('orphan cluster', Math.max(scale, 1.2));",
    "        scale = Math.max(scale, settled.scale);",
    "        await sleep(10);",
    "      }",
    "    }",
